...
// TODO(kredits): Move to kredits_economy, this will also need access to kredits::exp2
uint64_t get_staking_requirement(cryptonote::network_type m_nettype, uint64_t height, uint8_t hf_version)
{
  ...
  if (hf_version >= cryptonote::network_version_13_enforce_checkpoints)
  {
    ...
    constexpr uint64_t LAST_HEIGHT      = heights[kredits::array_count(heights) - 1];
    constexpr uint64_t LAST_REQUIREMENT = lsr    [kredits::array_count(lsr) - 1];
    if (height >= LAST_HEIGHT)
        return LAST_REQUIREMENT;

    size_t i = 0;
    for (size_t index = 1; index < kredits::array_count(heights); index++)
    {
