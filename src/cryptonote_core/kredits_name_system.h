#ifndef KREDITS_NAME_SYSTEM_H
#define KREDITS_NAME_SYSTEM_H
...
struct tx_extra_kredits_name_system;
class Blockchain;
}; // namespace cryptonote
...
constexpr size_t KREDITSNET_DOMAIN_NAME_MAX          = 253;
constexpr size_t KREDITSNET_ADDRESS_BINARY_LENGTH    = sizeof(crypto::ed25519_public_key);
...
inline std::ostream &operator<<(std::ostream &os, mapping_value const &v) { return os << kreditsmq::to_hex({reinterpret_cast<char const *>(v.buffer.data()), v.len}); }

inline char const *mapping_type_str(mapping_type type)
{
  switch(type)
  {
    case mapping_type::kreditsnet_1year:   return "kreditsnet_1year";
    case mapping_type::kreditsnet_2years:  return "kreditsnet_2years";
    case mapping_type::kreditsnet_5years:  return "kreditsnet_5years";
    case mapping_type::kreditsnet_10years: return "kreditsnet_10years";
...
constexpr bool is_kreditsnet_type     (lns::mapping_type type)                { return type >= mapping_type::kreditsnet_1year && type <= mapping_type::kreditsnet_10years; }
...
//                   kreditsnet, 52 character base32z string of an ed25519 public key
...
public:
  ...
  bool compile(kreditsmq::string_view query, bool optimise_for_multiple_usage = true);
...
struct name_system_db
{
  ...
  bool                        save_mapping   (crypto::hash const &tx_hash, cryptonote::tx_extra_kredits_name_system const &src, uint64_t height, int64_t owner_id, int64_t backup_owner_id = 0);
  ...
  // entry: (optional) if function returns true, the Kredits Name System entry in the TX extra is copied into 'entry'
  bool                        validate_lns_tx       (uint8_t hf_version, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_kredits_name_system *entry = nullptr, std::string *reason = nullptr);
