...
constexpr uint64_t COIN                       = (uint64_t)1000000000; // 1 KREDITS = pow(10, 9)
constexpr uint64_t MONEY_SUPPLY               = ((uint64_t)(10000000000000000000ULL) * 10); // MONEY_SUPPLY - total number coins to be generated
...
constexpr uint64_t EMISSION_DIVISOR           = 50000;
...
// TODO: For now we add 1 extra atomic kredits to the HF16 block reward, above; ultimately with pulse
...
constexpr uint64_t burn_needed(uint8_t /*hf_version*/, mapping_type type)
{
  ...
  switch (type)
  {
    ...
    case mapping_type::kreditsnet_1year: /* FALLTHRU */
    ...
    case mapping_type::kreditsnet_2years: result = 40 * COIN; break;
    case mapping_type::kreditsnet_5years: result = 80 * COIN; break;
    case mapping_type::kreditsnet_10years: result = 120 * COIN; break;
  }
  return result;
}
}; // namespace lns
