...
class Daemon(RPCDaemon):
    ...
    def __init__(self, *,
        ...
        if name is None:
            name = 'kreditsd@{}'.format(self.rpc_port)
        super().__init__(name)
        ...
        self.args = [kreditsd] + list(self.__class__.base_args)
        self.args += (
                '--data-dir={}/kredits-{}-{}'.format(datadir or '.', self.listen_ip, self.rpc_port),
                '--log-level={}'.format(log_level),
                '--log-file=kredits.log'.format(self.listen_ip, self.p2p_port),
    ...
    def ping(self, *, storage=True, kreditsnet=True):
        """Sends fake storage server and kreditsnet pings to the running kreditsd"""
        if storage:
            self.json_rpc("storage_server_ping", { "version_major": 9, "version_minor": 9, "version_patch": 9 })
        if kreditsnet:
            self.json_rpc("kreditsnet_ping", { "version": [9,9,9] })
